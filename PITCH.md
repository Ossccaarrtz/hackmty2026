...
**Autocrítica honesta: la verificación de acciones no cubre alucinaciones en el TEXTO de la respuesta.** Se encontró en producción (probando en vivo, no en teoría): el modelo podía inventar un monto en dólares plausible en su narrativa (ej. "$299/mes" para una fuga que en realidad cuesta $40/mes) cuando la herramienta no le daba ese dato exacto — sin ejecutar ninguna acción falsa, pero mintiéndole a la usuaria sobre un número real de su cuenta. Se agregó una segunda capa de verificación (`find_unverified_amounts` en `lambda_chat.py`) que compara cada cifra en dólares mencionada en la respuesta final contra los números reales que regresaron las herramientas ese turno — si no coincide, se le da al modelo una oportunidad de corregirse, y si tampoco pasa, se reemplaza automáticamente antes de que la respuesta salga. Es la aplicación literal del principio "verificar, no confiar en que el modelo se porte bien" — extendido del dinero que se mueve al texto que se genera.
...
**Nómina real de un tercero (`simulate_third_party_payroll`) es la evidencia más convincente en vivo de "acciones reales", pero comparte la misma limitación que Apartados: no mueve el score.** Es genuinamente más fuerte que insertar un depósito de prueba con nuestra propia cuenta — hay dos IDs de transacción real en Nessie, entre dos dueños distintos, y el reparto lo dispara solo el mismo webhook de siempre, sin ningún atajo de demo. Pero sigue siendo, en el fondo, la misma feature de presupuesto (Apartados) vista desde otro ángulo de entrada. Si hay tiempo en el pitch para un "bonus beat" después del guion de 3 minutos, este es el más fuerte para mostrar en vivo — pero no debe competir con la narrativa de Gym Co por los 3 minutos centrales.
...
Construimos 8 endpoints en vivo (score, transacciones, agente decisor, chat, notificaciones, apartados, reporte de confiabilidad, nómina real de un tercero). Eso es fortaleza de Technical Depth (25% de la rúbrica), pero Design & Experience también pesa 20% — y castiga un demo que se siente disperso. **Para el pitch de 3 minutos, una sola narrativa, la que ya tiene causa-efecto medible en vivo:**
