...
|---|---|---|
| `GET /signals?user_id=ana` | Score (0-100) + desglose de 4 features + alertas + liquidez + proyección | ✅ En vivo |
| `GET /transactions?user_id=ana` | Los movimientos crudos de Ana, con balance corriendo ya calculado | ✅ En vivo |
| `POST /simulation/advance-day?user_id=ana` | Avanza un checkpoint (Día 45 → 62 → 63 → 90), ejecuta la política de riesgo real, escribe de verdad en Nessie | ✅ En vivo, probado punta a punta |
| `POST /simulation/advance-day?user_id=ana&reset=true` | Reinicia la simulación a Día 0 (para ensayar la demo las veces que hagan falta) | ✅ En vivo |
...
- Reemplazar el mock por `fetch(`${API_BASE}/signals?user_id=ana`)`.
...
- ~~Pronóstico de gastos recurrentes por cadencia (Parte 1)~~ — **ya resuelto.** `signal_engine.forecast_upcoming_expenses` detecta gastos recurrentes por categoría (ej. renta cada ~30 días) a partir de la cadencia real observada — mínimo 3 ocurrencias, descarta categorías con intervalo irregular (coeficiente de variación > 50%). Expuesto en `/signals` (`upcoming_expenses`) y como tool de chat (`get_upcoming_expenses`). Probado en vivo originalmente contra la persona de prueba anterior: detectó su renta a 5 días con 100% de confianza (cadencia perfectamente regular) — mecanismo agnóstico de persona, sigue corriendo igual sobre la renta mensual de Ana. De regalo, se encontró y corrigió un bug de seguridad real: el guardrail de anomalía estaba apagado fuera de los checkpoints de `advance-day` desde que se construyó (ver README para el detalle).
...
- ~~Pendiente: no hay UI en frontend~~ — **resuelto**: la pantalla "Apartados" (página completa, no modal) permite crear/editar apartados, declarar el patrón de nómina, confirmar repartos pendientes, y ahora también disparar la demo de nómina de un tercero (ver más abajo). Los apartados y el patrón de nómina siguen siendo independientes de la simulación de checkpoints a propósito — no se borran en un reset.
- **Nuevo: nómina real de un tercero (`POST /envelopes/simulate-payroll`).** Hasta ahora, todo depósito de prueba se insertaba con nuestra propia llave de Nessie. Este endpoint mueve dinero real entre dos cuentas con dueños distintos (un segundo customer creado con la api key de otro integrante del hackathon, actuando como "empleador" — re-narrado a "Papa y mama" para Ana) y dispara el mismo camino reactivo de siempre (DynamoDB Streams → `lambda_notifier` → `verified_allocate_envelopes`) sin ningún atajo. Detalle completo, incluyendo la aclaración de arquitectura de por qué esto necesita escribir en DynamoDB además de mover el dinero en Nessie, en el [README](./README.md#backend-desplegado-ya-en-la-cuenta-oficial-de-aws-del-equipo), sección "Octavo endpoint en vivo". Por esto, `_handle_reset` de `advance-day` ahora SÍ limpia los repartos ya ejecutados (`category` empieza con `envelope:`) y los depósitos de esta demo (`income_third_party_demo`) -- sin esto, cada ensayo dejaría residuo permanente en el historial.
...
- ~~La tolerancia default del patrón de nómina es 25%~~ — **✅ corregido.** Se validó originalmente contra los 8 depósitos reales de la persona de prueba anterior (ya eliminada) y el ±25% fijo dejaba fuera **4 de 8** (ingreso freelance: $300-$720, media $565) — justo el perfil de ingreso irregular que el proyecto dice servir, rechazado por su propia verificación. `agent_actions.suggested_income_tolerance()` deriva la tolerancia del historial real de depósitos (±2 desviaciones estándar sobre la media, piso mínimo de 30% con poco historial) en vez de un porcentaje fijo arbitrario. `set_income_pattern` sigue aceptando un `tolerance_pct` explícito si alguien lo quiere forzar. Re-validado con Ana (patrón real declarado hoy: `expected_amount: 1581, tolerance_pct: 0.41` derivado — cubre sus 8 depósitos reales de $1,150-$2,100 MXN, mientras que un ±25% fijo habría rechazado los 2 extremos) + 7 tests nuevos (uno documenta el bug original a propósito, para que nadie baje la tolerancia a 25% fijo sin darse cuenta de lo que rompe). 40 tests en `test_agent_actions.py`.
...
- **Riesgo de "reloj de pared" — ✅ corregido antes de que se volviera un problema real.** El fix original del guardrail de anomalía (ver README) default'aba `as_of_date` a `date.today().isoformat()` cuando faltaba — funcionaba mientras "hoy" real coincidiera con el último checkpoint sembrado (2026-09-12), pero en cuanto el calendario real cruzara esa fecha, la referencia se habría adelantado sola más allá de cualquier dato real, sin que nadie lo notara hasta ver números raros. Corregido de raíz: `signal_engine.resolve_reference_date(as_of_date, purchases)` — nueva función compartida que, si no hay `as_of_date` explícito, usa la fecha más reciente que de verdad existe en los datos (`max(purchases)`), nunca el reloj real. Se revirtió el `date.today()` forzado en `agent_actions.get_full_signals` y `lambda_function.py`, y `evaluate_bills`/`detect_anomaly`/`forecast_upcoming_expenses` ahora usan la misma función. Verificado en vivo: `/signals` sin `as_of` da exactamente los mismos resultados que antes (score 64, `anomaly: false`, pronóstico de renta a 5 días) — el fix es invisible hoy y solo se nota en que seguirá funcionando igual mañana. Cerrado con 27 tests nuevos en `test_signal_engine.py` (60 tests en total entre los dos archivos de test).
...
## 8. Pivote de producto — Spark (persona Ana, estudiante) — ✅ completado, código y datos incluidos

Decidido el 2026-09-12 tras feedback de jueces/mentores sobre el modelo de negocio original. Razonamiento completo en [PITCH.md](./PITCH.md). Lo que sigue documenta cómo se ejecutó, en el orden en que se hizo — desde la primera pasada de solo-documentación hasta la purga final de la persona anterior (8.8):
...
**8.8 — Purga completa de la persona anterior (Mia) — ✅ hecho, backend + frontend + tests + datos reales**

Una vez validado que Ana funciona de punta a punta (score, fugas, activación, wallet share, chat, advance-day), se decidió eliminar toda referencia a Mia en vez de mantener las dos personas en paralelo indefinidamente — para no confundir a futuros colaboradores o agentes sobre quién es el cliente objetivo del proyecto. Alcance, en orden:
1. Defaults de `user_id` en todo el backend (`agent_actions.py`, todos los `lambda_*.py`) cambiados de `"mia"` a `"ana"`.
2. `lambda_advance_day.py` generalizado para derivar la fuga y el bill cancelado dinámicamente de `signals["alerts"]` en vez de tener el nombre/monto de "Gym Co" hardcodeado — funciona igual para cualquier persona sembrada, no solo la que se usó para construirlo.
3. `ACCOUNTS_BY_USER`/`CHECKING_ID`/`SAVINGS_ID` en `agent_actions.py` reducidos a solo la cuenta real de Ana (antes tenían a Mia como fallback).
4. Tests (`test_agent_actions.py`, `test_signal_engine.py`) actualizados: defaults a `"ana"`, y la aserción de tolerancia de ingreso recalculada a mano con los 8 depósitos reales de Ana (41% derivado, no una sustitución mecánica del número de Mia — un bulk-replace ciego habría dejado una aserción con el número equivocado).
5. Scripts y datos de seed específicos de Mia borrados del repo: `backend/signals-lambda/load_seed.py`, `seed/jarbis-seed.js`, `seed/jarbis-seed-output.json`, `backend/signal-engine.js` (prototipo Node ya no referenciado), `backend/signal-engine-output.example.json`.
6. **Los ~80 items reales de Mia en DynamoDB (`user_id="mia"`) se eliminaron permanentemente** — acción destructiva, ejecutada solo después de confirmación explícita del usuario y solo después de probar que Ana funcionaba de punta a punta. No se purgó el customer/cuentas de Mia en el sandbox de Nessie (footprint menor, técnicamente limitado a borrar por ID en esta versión del sandbox — ver "Hallazgos técnicos sobre la Nessie API" en el README — y de menor prioridad al no aparecer en ningún endpoint que lea de DynamoDB).
7. Frontend: "Mia"→"Ana", "Centinel"/"Centinel One"→"Spark", "Gym Co"→"FitZone Campus", clase CSS `.mia-avatar`→`.user-avatar`, prefijos de storage `centinel:`→`spark:`, defaults de `VITE_USER_ID`/`userId`→`ana` en todos los archivos de frontend (`api.js`, `main.jsx`, `styles.css`, `index.html`, `README.md`, tests, `.env.example`).
8. README.md/PLAN.md/PITCH.md (este pase): reemplazadas las afirmaciones de estado actual que ya eran falsas (ej. "el prototipo sigue corriendo sobre Mia"), reescrita la sección de seed con los números reales de Ana, y las narraciones de pruebas históricas específicas de Mia marcadas explícitamente como bitácora de ingeniería del momento en que se validaron, no como estado vigente.

Los 134 tests de backend y 13 de frontend se mantuvieron en verde durante todo el proceso.
