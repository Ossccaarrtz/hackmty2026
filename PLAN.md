...
**No accionable / cobertura de pruebas:** el backend sigue sin tests automatizados formales (solo pruebas manuales vía curl documentadas en README/PITCH). La auditoría señala que las funciones de `agent_actions.py` son exactamente donde se encontraron los bugs más serios de la sesión, y que 3-4 tests con mocks sobre las 4 funciones `verified_*` (parametrizando el escenario de anomalía activa) habrían atrapado el hallazgo de `verified_allocate_envelopes` en minutos sin desplegar nada. Queda como el candidato de mayor ROI si sobra tiempo — no se construyó en esta pasada porque la prioridad fue corregir lo ya encontrado, no ampliar cobertura preventiva.
