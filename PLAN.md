...
- ~~La tolerancia default del patrón de nómina es 25%~~ — **✅ corregido.** Se validó contra los 8 depósitos reales de Mia y el ±25% fijo dejaba fuera **4 de 8** (ingreso freelance real: $300-$720, media $565) — justo el perfil de ingreso irregular que el proyecto dice servir, rechazado por su propia verificación. `agent_actions.suggested_income_tolerance()` ahora deriva la tolerancia del historial real de depósitos (±2 desviaciones estándar sobre la media, piso mínimo de 30% con poco historial) en vez de un porcentaje fijo arbitrario. Con los datos de Mia da 54% — cubre sus 8 depósitos reales y sigue rechazando un depósito random fuera de rango (ej. $100 de un amigo). `set_income_pattern` sigue aceptando un `tolerance_pct` explícito si alguien lo quiere forzar. Probado en vivo contra el patrón real declarado hoy (`expected_amount: 565, tolerance_pct: 0.54` derivado, no adivinado) + 7 tests nuevos (uno documenta el bug original a propósito, para que nadie baje la tolerancia a 25% fijo sin darse cuenta de lo que rompe). 40 tests en `test_agent_actions.py`.
