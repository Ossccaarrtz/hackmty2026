...
- **Alertas estacionales (Parte 2 — diciembre, temporada alta, etc.)** — necesita al menos un ciclo anual completo de historial real para no ser un número inventado (viola la regla de "todo respaldado por datos reales" si se hardcodea). Evaluación de viabilidad con el tiempo restante delegada a un agente (ver abajo) — no construir sin que confirme que el costo de sembrar el historial adicional cabe en el tiempo que queda.
