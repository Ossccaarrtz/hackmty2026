...
Vite carga `.env` desde `frontend/`. Solo se necesitan `VITE_API_BASE_URL` y `VITE_USER_ID` (por defecto, endpoint AWS del equipo y `ana`). No copies `NESSIE_API_KEY` al frontend: todas las variables `VITE_*` son públicas. Reinicia Vite al cambiar configuración.

## Conexiones

- `GET /signals?user_id=ana`: score, cuatro componentes, tendencia real, anomalía, cobertura y alertas.
- `GET /transactions?user_id=ana`: todos los movimientos, ingresos y saldo `running_balance` calculado por el backend. Se muestran seis recientes y el detalle completo permite buscar y ver la gráfica del ledger.
- `POST /simulation/advance-day?user_id=ana`: pide autorización explícita antes de enviar, incorpora los textos literales de `new_actions` y registra el score del checkpoint. Después actualiza signals y transactions.
- `POST /simulation/advance-day?user_id=ana&reset=true`: pide confirmar el reinicio antes de enviarlo.
- `POST /chat/message`: chat real (Gemini + tool-calling). Las acciones que ejecuta se unen al mismo feed que `advance-day`.
- `GET /notifications?user_id=ana`: avisos generados por un webhook real (DynamoDB Streams) — panel en el home + modal completo.
...
- Reiniciar no revierte movimientos de ahorro **hechos vía chat en Nessie** (sí limpia el estado en DynamoDB y reactiva el bill de fuga detectado). El backend puede devolver éxito aunque no haya logrado reactivar el bill: revisar las alertas después del reinicio.
