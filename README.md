...
| API Gateway | Rutas `GET /signals`, `GET /transactions`, `POST /simulation/advance-day`, `POST /chat/message`, `GET /notifications`, `GET/POST /envelopes`, `POST /envelopes/income-pattern`, `POST /envelopes/confirm-allocation`, `POST /envelopes/simulate-payroll`, `GET /trust-report` agregadas al API `jarbis` ya existente. CORS configurado a nivel de API (necesario para el POST con JSON body del chat) |
